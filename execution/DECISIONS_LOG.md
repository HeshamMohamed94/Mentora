...
## D142 -- 2026-09-20 -- PHASE 6 T5 CLOSES: independent Codex second opinion on the provider boundary found 2 more genuine defects beyond the Opus review, both fixed (`0196bde`)

**Decision:** ran the `codex-reviewer` subagent, scoped narrowly per D139 § H5/D141 to `provider/*.kt`, `AiTutorModule.kt`'s Koin selection rule, and `AppConfig`'s key-handling fields -- an independent second opinion on top of, not a replacement for, the T5 Opus review already recorded in D141. Verdict: needs-attention, 2 medium findings, both real:

1. **`mapHttpError`'s own suspending body read had the same cancellation-swallowing risk F4 already fixed one level up** -- `readRemaining(...)` sits inside a plain `catch (Exception) { "" }`, so a client disconnect while reading a non-2xx error body would convert a real `CancellationException` into an ordinary `ApiException`, exactly the structured-concurrency violation F4 fixed in `complete()`'s outer catch chain, just at a different suspension point the first fix round didn't reach. Fixed with the identical catch-and-rethrow pattern.
2. **An empty `content_block_delta` could satisfy the "first token received" check**, so a `message_stop` arriving immediately after one reported a successful completion with zero actual assistant text -- a real, if narrow, second hole in the same "no usable response" invariant F2 (D141) already closed for the zero-deltas and clean-EOF cases. Fixed: only a non-empty text delta counts as the first token.

Also acted on a LOW test-rigor finding (the "key only in the header" test checked only the URL/body, not every other header) by strengthening that assertion to scan all captured headers. Codex confirmed none of D141's F1-F5/F7/F9 fixes were re-litigated and F6/F8's deferral was respected.

**Process note, recorded for transparency, not as a quality judgment:** Codex's own transcript showed it first attempted an internal specialized review-runner that failed to start in this environment, then fell back to direct manual investigation (reading the files, grepping, reading this log) to produce its findings -- the findings above are that fallback investigation's real conclusions, not something edited or supplemented by the relaying subagent.

**Verification:** 127/127 backend tests green (up from 125 after D141), independently re-verified this session -- both new fixes read in full against the real diff before committing, not solely trusted from the subagent's report. One test originally attempted for the cancellation fix was dropped after empirical confirmation that a pre-cancelled `ByteChannel` reads back as an empty body rather than propagating its cancellation cause through `readRemaining` under MockEngine's synchronous, already-buffered channels -- that fix is verified by code inspection and direct analogy to F4's already-tested case instead.

**T5 (the mandatory review checkpoint) is now CLOSED.** Both required reviews (primary Opus on the full diff, independent Codex on the provider/key boundary) are complete, and every finding from both is either fixed (F1-F5, F7, F9, plus the 2 Codex findings) or deliberately, explicitly deferred (F6, F8). T6 (documentation + config surface) is next.

## D143 -- 2026-09-20 -- PHASE 6 T6 done: documentation + config surface updated to reflect the real Anthropic provider

**Decision:** updated `backend/.env.example` (the Phase-1-era comment claiming "safe to leave unset, Phase 6 doesn't need a new config key" replaced with the real two-mode behavior and the two new optional keys documented, `.env` value itself left empty), added a new "AI Tutor provider" section to `backend/README.md` (the two modes, the always-on startup log line, the "no client ever holds a key" structural guarantee), updated `execution/INTEGRATION_CONTRACT.md` §§ 4 and 8 (the new `AI_TUTOR_UNAVAILABLE` 503 code recorded as an as-built addition needing no client change, per the existing `Unknown`-fallback precedent already used for `FORBIDDEN_CSRF`; § 8's Phase-1-era "stub only" language corrected to describe both modes, and the enrolled-course-context addition recorded as confirming what that section had already anticipated), and added a "PHASE 6" status section to `execution/CURRENT_STATUS.md` (task table T1-T9, explicit runtime-verified-vs-structurally-verified split per I3, the accepted/deferred gaps carried forward, and confirmation no `mobile/` file was touched).

**No code changed in this task** -- purely documentation, matching the real, already-implemented and already-tested state from T1-T5 (D139-D142). `AI_PROVIDER_API_KEY` remains empty in this environment; the docs say so plainly rather than implying otherwise.

## D144 -- 2026-09-20 -- PHASE 6 T7 done: cross-platform verification without a key (Website + Android), both PASS, zero client-code change

**Decision:** ran the plan's T7 checklist on both primary demo targets against the locally-running stub-mode backend -- no `AI_PROVIDER_API_KEY` involved, so nothing here exercises the real Anthropic path (that remains T8, gated). Website was verified directly by this session via Chrome browser automation; Android was delegated to a background agent driving a real debug APK on an emulator via adb, per the user's own instruction that Android must be genuinely exercised, not statically inspected.

**Website results (all PASS):** navigation to `/en/app/ai-tutor` and `/ar/app/ai-tutor`; first-open welcome + 5 quick-action chips; three-dot "Thinking" loading bubble; all 5 quick actions round-trip; killing the backend mid-conversation produces the clean generic error bubble ("Something went wrong sending your message. Try again.") with the user's message preserved, no crash, no raw error code (B3/E3); restarting the backend and clicking "Try again" succeeds and the conversation resumes normally; Arabic/RTL (title, quick actions, input placeholder, send-arrow direction all correctly mirrored, F2); Light/Dark (dark theme renders with correct contrast in both English and Arabic, F3).

**Android results (all PASS, via background agent, tested as `student2@mentora.dev` against the existing hardcoded `10.0.2.2:8080` dev config -- no config change needed):** B4 (first-open welcome bubble + chip row, not a full-screen empty state); B6 (all 5 quick actions incl. "What should I learn next?"); B2 (three-dot Thinking pulse observed live in a screenshot); B3 (killed backend mid-send -> permanent user bubble + inline error + Retry card, message never lost; restarted backend -> Retry succeeds; a second, independently-triggered error also retried correctly, confirming per-turn retry isolation, not global recovery); F2 (Arabic/RTL: title, chips, placeholder, bubble alignment, bottom-nav order all correctly mirrored); F3 (Dark theme combined with Arabic: fully dark-themed, correctly tinted, no default-Android unstyled elements). E3 (`AI_TUTOR_UNAVAILABLE`) was verified structurally by code rather than by forcing a live 503 -- `ApiErrorCode.kt`/`ApiErrorCopy.kt` confirm the unmapped code falls through to the same generic `Unknown`-error/Retry path already directly observed live for connection failure in B3; forcing a real 503 would have required editing backend provider config, which the agent correctly treated as out of scope for a verify-only pass.

**Scope discipline confirmed:** `git status`/`git diff --stat` show zero files changed under `web/` or `mobile/` on either platform -- this was a verify-only pass, exactly as planned, with no client code needed to support the already-built AI Tutor UI against the now-real backend.

**One incidental, unrelated finding, explicitly out of scope, not investigated further:** on Android's Course Details screen, the "Login to Enroll" CTA doesn't immediately refresh to "Enroll" right after logging in via the auth-gate flow (a stale auth-state read on that screen). Not an AI Tutor defect, not part of Phase 6, not a blocker -- recorded here only so it isn't lost, worth a look in a future pass.

**Next:** the T8 gate -- T8 (live runtime verification against the real Anthropic API) remains hard-blocked until the user supplies a real `AI_PROVIDER_API_KEY` and confirms the exact Anthropic model id (System Design § 24's open question). Per explicit standing instruction, this will not be worked around, simulated, or assumed -- T9's final acceptance audit will report T8's items as BLOCKED/NOT TESTABLE, not as PASS, until a real key is provided.

## D145 -- 2026-09-20 -- PHASE 6 T9 done: final acceptance audit closes the phase (T8 gate excepted)

**Decision:** ran the T9 final acceptance audit specified at Phase 6 kickoff -- a criterion-by-criterion matrix (implementation evidence -> test evidence -> PASS/PARTIAL/NOT TESTABLE), not a blanket "looks done" declaration. Every row of `execution/PHASE_6_ACCEPTANCE_CRITERIA.md`'s A-J checklist (originally authored as a draft at kickoff, D139) was updated in place against the real, final end-of-phase state, re-verifying rather than re-trusting: re-ran the full backend suite fresh (127/127 green, 0 failures, 0 errors, `--rerun`, not relying on Gradle's up-to-date cache), re-confirmed `backend/.env` is gitignored and has never been committed (`git log --all -- backend/.env` empty) and that `.env.example`'s `AI_PROVIDER_API_KEY` value has only ever been empty across its entire git history, re-read the actual timeout-wiring code (`AiTutorModule.kt`'s `HttpTimeout` install, confirmed genuinely config-driven, not just declared and unused) before marking A6 PASS rather than assuming it from the config field's existence alone.

**Result:** every criterion is PASS, PASS-STRUCTURAL, N/A, or an explicitly ACCEPTED GAP, except seven items that are genuinely, only NOT TESTABLE without a real Anthropic credential -- **A1, A3, B1, C3, D3, F4, G4** -- all gated on the single unmet precondition in § I3. No other open defect, regression, or undone item exists anywhere in the A-J checklist. Also added a Phase 6 entry to `execution/PHASE_HANDOFF.md` (the standard 10-subsection format used for Phases 1-4) and finalized `CURRENT_STATUS.md`'s top banner and Phase 6 task table to reflect this closure.

**Environment cleanup performed as part of this audit:** stopped the two local dev processes (backend stub-mode server, web dev server) started for T7's manual QA and removed their session-scratch log files (`backend/.t7-backend.log`, `web/.t7-web.log`, never committed) -- these were temporary verification aids, not part of the shipped system.

**What this does and does not mean:** Phase 6's own T9 gate (this audit) is CLOSED -- the phase's implementation, review, documentation, and everything-testable-without-a-key verification are genuinely, completely done. Phase 6 as a whole is **NOT YET fully closeable** only because T8 (live runtime verification against the real Anthropic API) remains hard-blocked on a missing external credential this session has no authority to fabricate, simulate, or assume -- per this project's explicit standing instruction (I3). This is a real, disclosed, external blocker, not a quality gap or an unfinished task. **Phase 7 has not been started and must not begin without the user's own separate, explicit approval**, regardless of T8's eventual resolution.

## D146 -- 2026-09-20 -- PHASE 6 CLOSURE: user explicitly DEFERS real Anthropic live-provider verification (T8); Phase 6 implementation declared COMPLETE for the portfolio/demo scope; Phase 7 approved to start

**User decision, recorded verbatim in substance:** the user explicitly chose to skip the real Anthropic live-provider test (T8) for this project, stating this is a local portfolio/demo project and they do not want to spend time, credentials, or API cost on live Anthropic verification right now. The user was explicit that: no AI_PROVIDER_API_KEY should be requested again; no real key should be created, requested, exposed, or configured; no real external AI-provider call should be made; the lack of a real provider credential must no longer block the rest of the Mentora project; and none of Phase 6's existing implementation, review, structural verification, mock/test evidence, or documentation may be weakened or fabricated to manufacture a false PASS.

**What changed as a result of this decision (documentation only, zero code touched):**
1. `execution/PHASE_6_ACCEPTANCE_CRITERIA.md` -- reworded every one of the seven affected criteria (A1, A3, B1, C3, D3, F4, G4) from "NOT TESTABLE (blocked on the T8 gate)" to "NOT TESTABLE -- real AI provider credential intentionally not configured by user decision (D146)". This is a *reframing of the same honest status*, not an upgrade: none of the seven were changed to PASS, and every PASS-STRUCTURAL designation (where real mock/fake/integration test evidence genuinely exists, e.g. A1's provider implementation, A3's SSE parsing, C3's enrolled-course-scoping) was left exactly as it was. A new "PHASE 6 CLOSURE DECISION" section states plainly: PHASE 6 IMPLEMENTATION: COMPLETE (for the portfolio/demo scope); LIVE PROVIDER VERIFICATION: DEFERRED BY USER DECISION; and explicitly does NOT claim "PHASE 6 ACCEPTANCE: PASS" against the checklist's original acceptance contract, which required real-provider verification (§ I3) -- that contract's live-provider clauses are now knowingly, permanently waived by the user, the only party with authority to waive them.
2. `execution/CURRENT_STATUS.md` and `execution/PHASE_HANDOFF.md`'s Phase 6 entry -- updated to the same effect (see their own edit history for this date).
3. No `backend/`, `web/`, or `mobile/` file was touched -- this decision changes nothing about what was built, reviewed, or tested; it only changes how the one remaining gap is described and whether it continues to gate later work.

**Security re-confirmed (not re-audited from scratch -- these facts were already conclusively established in D144/T9 and are simply re-stated here since the user asked for confirmation):** no AI provider secret exists in source control (`backend/.env` gitignored, never committed, full-history scan clean; `.env.example`'s `AI_PROVIDER_API_KEY` value has only ever been empty across its entire git history); Android/Web clients contain no provider secret and never reference one; the backend remains the sole AI-provider boundary (`AiProvider`/`Anthropic*.kt` are the only files in the codebase that reference a key or `api.anthropic.com`). No new key was created, requested, exposed, or configured to reach this decision or in response to it.

**Why this is not "PASS" and why that's the correct call, not a discrepancy:** the original `PHASE_6_ACCEPTANCE_CRITERIA.md` was authored (D139) against an acceptance contract that explicitly required real-provider runtime verification for A1/A3/B1/C3/D3/F4/G4. The user's decision here waives that requirement for this project going forward, but does not retroactively make those seven checks true -- they remain honestly unverified against a real provider, now by deliberate choice rather than by temporary absence of a key. "PHASE 6 IMPLEMENTATION: COMPLETE, LIVE PROVIDER VERIFICATION: DEFERRED BY USER DECISION" is the accurate summary; "PHASE 6 ACCEPTANCE: PASS" against the original contract would misrepresent what was actually verified.

**Project continuation, explicitly approved by the user:** Phase 5 (iOS) remains DEFERRED / PARTIALLY IMPLEMENTED, unaffected by this decision, not to be resumed. Primary supported/demo targets remain Website, Android, Backend, KMP shared core. Neither the deferred iOS work nor the deferred real Anthropic live test are blockers for Phases 7-8. **Phase 7 ("Full Integration") is explicitly approved to start following this documentation pass**, per the user's own instruction, using the existing mock/fake/test-provider infrastructure wherever AI Tutor runtime behavior needs exercising in Phase 7 -- no real Anthropic credential is to be required or requested for Phase 7. Phase 8 remains NOT approved and must not start without the user's own separate, explicit approval, exactly as Phase 7 was gated before this message.

## D147 -- 2026-09-20 -- PHASE 7 KICKOFF: Acceptance Criteria, System Design, Implementation Plan authored; four flagged judgment calls resolved

**Decision:** authored `execution/PHASE_7_ACCEPTANCE_CRITERIA.md` (sections A-J plus a scope-reconciliation section H, grounded in `MASTER_IMPLEMENTATION_PLAN.md`'s Phase 7 line, `PRODUCT_SPEC.md § 10`, `IMPLEMENTATION_ROADMAP.md § M16`, and this project's own D146), then dispatched the `architect` subagent to inspect the real codebase and produce `execution/PHASE_7_SYSTEM_DESIGN.md` (12 sections) and `execution/PHASE_7_IMPLEMENTATION_PLAN.md` (T0-T13). Both were reviewed in full before proceeding.

**Scope reconciliation (§ H1, restated for the decision log):** `MASTER_IMPLEMENTATION_PLAN.md`'s original Phase 7 line named Web/Android/iOS and M16 named four local-networking targets including iOS. Per the already-recorded Phase 5 freeze and this Phase 7 kickoff's own explicit instruction, **iOS is excluded** from Phase 7's cross-client verification scope -- narrowed, not silently overridden.

**Real recovery finding confirmed:** `.github/workflows/` contains exactly `ios-ci.yml`. No backend/web/android CI exists, despite M16 mandating it. This is genuine, already-locked, undone scope for this phase, not new work invented for it.

**Four flagged judgment calls, resolved as Lead/Supervisor per this project's established delegation authority (the same authority already exercised for D139 § H5 and other reconciliations), each because the architect explicitly flagged it as needing a call rather than making it unilaterally:**

1. **F1 -- the `isEnrolled` pagination divergence** (Web derives it from `/enrollments?limit=100`, first page only; Android pages every `nextCursor`; identical below 100 enrollments, divergent above). **Decision: record as a disclosed, accepted divergence (the architect's own primary recommendation), do not fix in Phase 7.** At demo scale (a handful of seeded enrollments per account) this can never actually manifest; spending a code change on it would be exactly the kind of scope creep J5 forbids for a criterion (A6) that is otherwise already true by construction. If it is ever fixed, the correct fix is a real backend `isEnrolled` field (removing the need for either client to derive anything), not a Web-side pagination patch -- and that is a backend contract change explicitly out of Phase 7 scope, not a Phase 7 task.
2. **F2 -- the fifth M16 workflow, `tokens-ci.yml`.** **Decision: build it (keep plan task T5).** It traces to M16's literal "all five GitHub Actions workflows" text, costs ~40 lines and seconds of runtime, is fully additive/reversible, and closes a real, previously uncovered gap (Web's generated token files have no drift gate today, unlike Android's and iOS's). Declining it would leave a known M16 deliverable undone for a reason weaker than the one that justifies building it.
3. **F3 -- the Android `:androidApp:lintDebug` baseline.** No decision needed yet -- genuinely unknown until run (T0's own job). Resolved empirically during execution per the System Design's three-option rule (zero errors -> ship as-is; a few trivial ones -> fix mechanically; otherwise -> a recorded, committed baseline).
4. **F4 -- the G1/G2 localization/theme walk-matrix size for T10/T11.** **Decision: the reduced-but-stated fallback** -- en-Light and ar-Dark (the two maximally-different combinations) walked in full for both clients, plus targeted spot-checks of en-Dark and ar-Light limited to the highest-risk screens (Course Player, Quiz, Checkout, AI Tutor). This mirrors the exact precedent already set in Phase 4 (D94: 16/18 screens in Arabic+Dark, not an exhaustive sweep) rather than inventing a new, heavier standard for this phase.
5. **F5 -- WebKit stays out of CI.** No new decision -- already covered by the System Design's own D-E2E-BROWSER call (chromium only, D64's real non-product cookie-policy incompatibility) and D64 itself. Carried forward, not re-litigated.

**Not yet resolved, deliberately:** whether `codex-reviewer` runs alongside T7's mandatory Opus review depends on what that review actually finds in the C4 fix -- per this project's standing policy, that call is made at T7, not pre-decided here.

**Next:** T0 -- baseline green-run across every existing suite plus the three CI-blocking preconditions (Android lint, token-pipeline drift, Playwright local pass count), establishing the regression floor every later Phase 7 task is measured against.

## D148 -- 2026-09-20 -- PHASE 7 T0 done: fresh baseline established; all three CI preconditions resolved; 2 real E2E defects found and fixed

**Decision:** ran every existing suite fresh (`--rerun-tasks`, not from Gradle's cache) to establish the real regression floor for Phase 7, and resolved all three preconditions the System Design flagged as blocking T3/T4/T5.

**Fresh baseline confirmed:** backend 127/127; `:shared:testDebugUnitTest` 249/249; `:androidApp:testDebugUnitTest` 241/241; web `npm run lint` (0 errors, 1 known pre-existing `<img>` warning, D-somewhere-earlier), `lint:logical-properties` clean, `validate:design-to-code` clean (37 screens/6 patterns/11 shared files), `typecheck` clean, `npm run build` succeeds. `actions/setup-node`'s current major resolved via the GitHub API: **v7**.

**Precondition 1 (Android lint, blocks T4) -- RESOLVED, no baseline needed.** `:androidApp:lintDebug` initially found **17 errors, 102 warnings**. Applied Design 5.3's option 2 (fix mechanically, since every error was genuinely trivial and behavior-preserving) rather than option 3 (a committed baseline file): 13x `UnsafeOptInUsageError` (both Media3 files used `kotlin.OptIn` instead of `androidx.annotation.OptIn` -- a well-known Kotlin/AndroidX-annotation gotcha where AGP lint's `UnsafeOptInUsageDetector` only recognizes the latter; fixed by adding `import androidx.annotation.OptIn` to shadow the default `kotlin.OptIn` resolution, zero behavior change); 2x `UnrememberedMutableState` in a test file (wrapped in `remember { }`, the objectively correct fix); 1x `MissingTranslation` for `app_name` (added to `values-ar/strings.xml` with the identical value -- it's a brand name, deliberately not translated); 1x `BidiSpoofing` (a deliberate, already-commented LRI/PDI bidi-isolation fix for RTL sequence numbers, false-positive-flagged by lint -- suppressed with `@Suppress("BidiSpoofing")` and an explanatory comment). `:androidApp:lintDebug` now passes with **zero errors**. `:shared` 249/249 and `:androidApp` JVM 241/241 unchanged after the fixes. Commit `4f95453`.

**Precondition 2 (token-pipeline drift, blocks T5) -- RESOLVED, clean.** `node tools/token-pipeline/generate.js` followed by `git status` showed zero drift. T5 (`tokens-ci.yml`) can proceed without a pre-landing regeneration.

**Precondition 3 (Playwright local pass count, blocks T3) -- RESOLVED, but only after finding and fixing 2 real defects, not by re-running until green.** The suite was NOT cleanly 19/19 on the first several attempts (4-9 failures depending on run conditions and worker count) -- initially suspected as rate-limit contamination from back-to-back runs or dev-mode-vs-prod-mode timing differences, both ruled out by isolation testing. The two real, reproducible root causes, found by isolating individual specs and reading the actual product/test code, not by guessing:
1. **`useUpdatePosition`'s fetch (`web/src/lib/api/progress.ts`) was fire-and-forget with no `keepalive`.** A real user (or `04-start-resume-course.spec.ts`) pausing a video and immediately navigating away could lose the position-update request to browser navigation-cancellation, since the mutation is never awaited by the caller (`course-player-screen.tsx`'s `savePosition` calls `.mutate()`, not `.mutateAsync()`, by design -- fire-and-forget is the correct UX for a background position report). Fixed by adding `keepalive: true` to that one call site only (not the shared `apiFetch` client globally, to avoid affecting unrelated endpoints like media uploads) -- a genuine product robustness improvement, not merely a test accommodation. The test itself was also given a `page.waitForResponse(...)` wait for the position-save's response before simulating "leaving", since the app never awaits its own mutation and the test's own setup needs to be deterministic regardless.
2. **`08-language-switch.spec.ts` used an ambiguous `getByText("الإعدادات")` locator** matching both the sidebar nav's current-page label and the page's own `<h1>` heading -- both correctly, simultaneously visible; a real product screen would be wrong to hide either. Narrowed to `getByRole("heading", { name: "الإعدادات" })`, the same specific pattern the file already used correctly for the English assertions.

Both fixes verified: **18-19/19 passed, one self-healing flake** (`helpers.ts`'s `completeAllLessons`, a "Mark Complete" -> "Completed" UI-state wait that occasionally needs its already-configured `retries: 1` -- itself an existing, intentional design choice per `playwright.config.ts`, not something this task introduced or is expected to eliminate), **confirmed green twice consecutively** against a real production build (`next build` + `next start`, matching exactly what `web-ci.yml`'s E2E job will run -- an earlier attempt against `npm run dev` was discarded as non-representative). Commit `a54277f`.

**Why these fixes belong in T0 despite T0's own scope line saying "no code change":** both trace directly to a named acceptance-criteria row (I2, B1) and to `PHASE_7_SYSTEM_DESIGN.md` section 5.3's own explicit allowance for exactly this situation (fix mechanically when trivial, rather than baseline/skip). Discovering them was T0's whole purpose -- establishing a *real*, not assumed, precondition state for T3/T4 -- and leaving them unfixed would have meant either shipping `web-ci.yml` red on day one (contradicting T3's own completion gate) or silently declaring a false "19/19 baseline" that this session's own investigation had just disproven. Recorded here in full per J5's scope-discipline rule; not folded in silently.
...
## D151 -- 2026-09-20 -- PHASE 7 T3 done: `web-e2e` green twice consecutively, after finding and fixing a real bug that meant the backend's fat jar was NEVER actually runnable

**Decision:** added the `web-e2e` job to `web-ci.yml` (MongoDB rs0, backend fat jar, seeded data, Next.js production build/start, Playwright chromium) per `PHASE_7_SYSTEM_DESIGN.md` § 4.2-4.4.

**Real, significant bug found on the first CI attempt (`35531585183`) and confirmed to reproduce identically on this Windows host, not a CI-environment quirk:** `java -jar mentora-backend.jar` crashed immediately with `IllegalArgumentException: Neither port nor sslPort specified` -- meaning `application.conf` (this backend's HOCON config, which sets `ktor.deployment.port` and the module list) was never read at all. `-Dconfig.trace=loads` produced zero output, and pointing `-Dconfig.file=` directly at an extracted copy of the file failed identically, ruling out a classpath/resource-discovery problem. Root cause, found by reading the fat jar's own `META-INF/services/io.ktor.server.config.ConfigLoader` file: it contained only `io.ktor.server.config.yaml.YamlConfigLoader` -- the HOCON loader entry was missing. `shadowJar`'s default merge strategy keeps exactly one arbitrary copy of a `META-INF/services/*` file when multiple dependency jars each provide their own (here: Ktor's YAML and HOCON config-loader modules each register their own provider under the same service file path), instead of merging their contents -- so the fat jar had silently, permanently lost the ability to read `.conf` files. This had never been noticed before because **every single backend run in this project's six-plus-phases history used `./gradlew run`**, which uses the unpacked classpath (classes + resources as directories, not a shaded jar) and was never affected. `buildFatJar`'s actual output had, in effect, never been runnable.

**Fix:** `backend/build.gradle.kts` -- `tasks.withType<ShadowJar> { mergeServiceFiles() }`. Verified directly (not just trusted): rebuilt the jar, confirmed both `YamlConfigLoader` and `HoconConfigLoader` now appear in the merged service file, ran the jar locally with the exact CI env vars, and confirmed the real startup log sequence (AppConfig summary, AI Tutor stub-mode line, Koin, `/healthz` -> 200) that was completely absent before. Backend test suite unaffected (127/127, task `UP-TO-DATE` -- this change touches only the `shadowJar` task, nothing `test` depends on).

**Verification, real CI, green twice consecutively as T3's own elevated completion gate requires:** run `35532267141` -- <https://github.com/HeshamMohamed94/Mentora/actions/runs/35532267141> -- **19/19 passed**, no flake. Run `35532576017` -- <https://github.com/HeshamMohamed94/Mentora/actions/runs/35532576017> -- **19/19 passed** again, no flake. Neither run needed Playwright's own `retries: 1` to reach green. No spec was skipped, deleted, or weakened to get there -- every fix (this one, plus D148's two E2E fixes) was a real defect, found and corrected at its actual root cause.
...
## D155 -- 2026-09-20 -- PHASE 7 T7 CLOSES: independent Opus review of T1-T6 found 2 real MEDIUM issues, both fixed; no `codex-reviewer` second opinion needed

**Decision:** ran the `reviewer` (Opus) subagent over the full real diff of T1-T6 (`60e71b7..165b232`), independently re-verifying every claim in D148-D154 rather than trusting them (re-ran the test suites and lint itself, re-fetched and inspected all six claimed CI runs via `gh`, inspected the fat jar's actual `META-INF/services/*` contents and scanned all 292 backend dependency jars for other collision-prone resources, traced `MentoraNavHost`'s three auth-state-transition branches directly in source). Verdict: **no HIGH findings**; every D148-D154 account confirmed accurate. 2 MEDIUM findings, both real and both fixed; 8 LOW findings, 5 fixed and 3 recorded as accepted/deferred.

**MEDIUM 1 -- `web-ci.yml`'s path filters omitted `backend/**`, so the only job that actually packages-and-runs the backend jar (`web-e2e`) would never trigger on a backend-only change** (a real gap: `backend-ci.yml` compiles/tests/assembles the jar but never launches it -- exactly the blind spot that let D151's shadowJar bug ship unnoticed for as long as it did). **Fixed:** added `'backend/**'` to `web-ci.yml`'s `paths:` filters, with the reasoning recorded in the file's own header.

**MEDIUM 2 -- `CourseDetailsViewModel.loadCourse()`/`LearningPathDetailsViewModel.loadPath()` had no in-flight-request cancellation, and the C4 fix added a THIRD independent trigger (`onAuthenticationChanged`) alongside `init` and `onRetry`.** Concrete failure path the reviewer traced: a guest-mode load still in flight when auth state flips could complete AFTER the newly-triggered authenticated load, silently overwriting the correct result with a stale one -- recreating C4's own symptom via a different path. Not a regression C4 introduced (the hazard already existed for `onRetry`/locale-change), but C4 gave it a third, more likely-to-race trigger. **Fixed:** both `loadCourse()` and `loadPath()` now track their launched `Job` and `cancel()` any prior one before starting a new load, so only the latest call's result can ever land regardless of completion order.

**5 LOW findings fixed:**
- `tokens-ci.yml`'s drift check used `git diff --quiet`, which only sees modifications to already-tracked files -- a brand-new, entirely untracked generated output (a future new theme file or platform target) would pass vacuously. Changed to `git status --porcelain`.
- `web-static`'s job name still said "+ build" after D150 removed that step. Renamed to `Web lint + type-check`; the file's own top-of-header description corrected to match.
- `LearningPathDetailsViewModel.onAuthenticationChanged` omitted the `followInFlight` guard that `reloadOnLocaleChange`'s own identical reload call already uses for the identical reason (an in-flight follow/unfollow result would be silently dropped). Currently unreachable in practice, but D144 itself was born from exactly that kind of "currently unreachable" reasoning -- made consistent.
- `execution/CURRENT_STATUS.md`'s Phase 7 banner still read "KICKOFF (T0 IN PROGRESS)" while the task table beneath it correctly showed T0-T6 done. Corrected.
- (`web-ci.yml`'s header/comment text, folded into the MEDIUM 1 fix above.)

**3 LOW findings recorded, not fixed, with reasoning:**
- The `keepalive: true` fix (D151) no longer has any automated test coverage, because `04-start-resume-course.spec.ts`'s own deterministic fix (`waitForResponse` before navigating) removed the exact race it exercises. Deliberately accepted: re-introducing that race into the spec on purpose would trade a deterministic, reliable CI suite for coverage of an edge case already covered by code review and by the fix's own directness (one line, `RequestInit`'s own documented `keepalive` field, correctly plumbed through per the reviewer's own trace of `client.ts`'s spread order).
- The 106 instrumented Android tests (`connectedDebugAndroidTest`) were not re-run after either T0's `NavigationShellTest.kt` edit or T6's behavioral change, since instrumented tests are out of CI by design (D-EMU) and require a real emulator. The reviewer's own source-level analysis concluded both edits are behavior-preserving, but this is analysis, not a run. **Folded into T11's scope** -- the live emulator walk will exercise this class of behavior directly, and running the instrumented suite there closes the gap for real rather than by inference.
- T3's own written completion gate (d) said "no product source file changed," but `0282f91` changed `backend/build.gradle.kts`. Already fully disclosed in D151 with its reasoning; T13's final audit will state this explicitly as a gate-text-vs-outcome note rather than letting the plan read as if met verbatim.

**Verification after the fixes:** `:shared` 249/249, `:androidApp` 246/246 (unchanged counts -- these are robustness fixes, not new test-visible behavior beyond what the existing 5 C4 tests already assert), `:androidApp:lintDebug` still 0 errors, token-pipeline regeneration still produces zero drift. `git status --short` after all fixes touches exactly 5 files: `.github/workflows/tokens-ci.yml`, `.github/workflows/web-ci.yml`, `execution/CURRENT_STATUS.md`, and the two ViewModel files -- no test file needed to change, since the existing C4 tests already exercise the code paths these fixes touch without depending on the old racy behavior.

**`codex-reviewer` second opinion: not run, per the reviewer's own explicit recommendation, endorsed here.** T6 is auth/session-adjacent (a listed trigger for an additional independent opinion), but every finding was either resolved with a confirmed root cause and a small, well-understood fix, or explicitly deferred with reasoning -- nothing serious or uncertain remains that a second independent pass would likely resolve differently. Per this project's standing policy (weekly Codex usage is limited, never invoked speculatively), skipping it here is the correct call, not a shortcut.

**T7 (the mandatory review checkpoint) is now CLOSED.**

**Next:** T8 -- the cross-client parity harness (`tools/cross-client-check/`).

## D156 -- 2026-09-20 -- PHASE 7 T8 done: cross-client parity harness built, all 8 assertion groups PASS on 3 consecutive real runs

**Decision:** built `tools/cross-client-check/check.js` + `README.md` exactly per `PHASE_7_SYSTEM_DESIGN.md` § 9 and `PHASE_7_IMPLEMENTATION_PLAN.md`'s T8 scope: plain Node 22, zero dependencies, two transports (`WebTransport` -- cookies + the `X-Requested-With: mentora-web` CSRF header, matching what the Website sends; `SdkTransport` -- `Authorization: Bearer`, matching `KtorHttpClient` in `shared/src/commonMain/kotlin/com/mentora/shared/network/`), one throwaway `@xclient.mentora.test` student registered fresh each run, sharing one access token across both transports. Every real HTTP route path used (`/api/v1/courses/{id}/checkout/complete`, `/api/v1/enrollments`, `/api/v1/courses/{id}/progress`, `/api/v1/courses/{id}/lessons/{lessonId}/complete`+`/position`, `/api/v1/courses/{id}/quiz`+`/attempts`+`/attempts/latest`, `/api/v1/certificates`, `/api/v1/ai-tutor/conversation`+`/messages`, `/api/v1/learning-paths/{id}`+`/follow` (POST to follow, **DELETE** to unfollow -- not a toggle)) was confirmed against the actual route/service source before writing a single line of the harness, not guessed or copied from documentation.

**Real bugs found and fixed while getting the first clean run (all in the harness itself, not backend/product code):**
1. **Response-envelope unwrapping bug.** Both transports' `call()` returned the raw `ApiSuccess<T>` envelope (`{data, meta}`) instead of unwrapping to `.data` (`common/Responses.kt`'s actual shape) -- every assertion after registration failed with "expected array" errors. Fixed by unwrapping in both `WebTransport.call` and `SdkTransport.call`, with a special case for the AI Tutor's message-post endpoint, which is NOT a JSON envelope at all (`respondTextWriter`, a real streamed `text/plain` token stream per `AiTutorRoutes.kt`) -- unwrapping only applies when the response is actually JSON.
2. **AI Tutor validation gap in the harness's own request.** `SendAiMessageRequest` validation requires `courseId` and `lessonContextId` together (`COURSE_AND_LESSON_REQUIRED_TOGETHER`) -- the harness's first draft sent `courseId` alone. Fixed by sending both.
3. **A4 (certificates) was never actually exercised in the first working run** -- `CertificateService.checkAndIssueIfComplete` requires `completedLessonCount == totalLessons` in addition to a passed quiz, and the harness's A2/E1 group only completes ONE lesson (that's its own point -- proving a single cross-transport write is visible). Rather than leave A4 permanently NOT-EXERCISED (technically honest, per the plan's own hard rule, but a weak first run), added a small setup step that completes every remaining lesson in the seeded course before the quiz attempt, so the real precondition is genuinely met and A4 asserts for real.

**First and second full runs (after the fixes), against the live local stack (`start-mentora.ps1`-equivalent: MongoDB rs0 healthy, backend started via `./gradlew run`, `seedDemoData` already applied from an earlier task):** all 8 design-named assertion groups (C1, D1, A1, A2/E1, E2/A3 (reported as 2 lines), A4, A5, E3) reported **PASS** -- zero FAIL, zero NOT-EXERCISED, on 2 consecutive runs with 2 different fresh throwaway accounts. A3's quiz attempt scored 100/passed=true both times (SeedData.kt's `question()` helper always emits `[correct, incorrect]` in that order, confirmed by reading the actual seed call sites before relying on it). A4 issued and matched 1 certificate both times. Full transcripts are in this task's session log; not re-pasted here for length, but reproducible by anyone via `node tools/cross-client-check/check.js` against a freshly seeded local stack.

**One transient, unexplained Node/Windows native crash, disclosed rather than hidden:** the very FIRST attempt (before fix #1 above was applied) crashed on exit with `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING), file src\win\async.c, line 94` -- a libuv-level assertion inside Node 24.19.0's own runtime, not a JavaScript exception, immediately after the script's own `report.print()` returned and `process.exit(1)` was called. It did not recur on any of the 3 subsequent runs (2 clean full runs plus the intermediate partial run while fixing bug #2), all of which exited normally with the correct code. This looks like a known class of Node/libuv teardown race on Windows when `process.exit()` is called while `undici`/`fetch` still holds pending keep-alive socket handles, not a defect in the harness's own logic -- but since it has not recurred and a single occurrence cannot be root-caused with confidence, it is recorded here as an observed-once, not-reproduced anomaly rather than silently forgotten. If it recurs in T11/T13, this entry is the reference point.

**Files touched.** `tools/cross-client-check/check.js` (new), `tools/cross-client-check/README.md` (new). `git status --porcelain` confirms these are the only two new files; no production source file changed, matching the plan's completion gate.

**Independent review:** not run for this task specifically -- consistent with how T1-T6 (each individually substantial) were bundled into the single T7 consolidated Opus review rather than reviewed one-by-one. This is a QA/test-tooling script (zero production-code change, no auth/authz/security/migration surface), the harness was verified end-to-end by 2 clean consecutive real runs against the live stack (not just self-reported), and T13's final acceptance audit is the phase's closing verification gate. Judged not to meet the bar for an individual review under this project's standing routing rules.

**T8 done. Next:** T9 -- static confirmations pass (A6, C3, D2, F2, I4), no code change expected.

## D157 -- 2026-09-20 -- PHASE 7 T9 done: all 5 static confirmations re-run and recorded; F1 (`isEnrolled` divergence) decision taken explicitly

**Decision:** re-ran, for real, the five checks `PHASE_7_SYSTEM_DESIGN.md` § 10 had already pre-asserted from source inspection, so each moves from "the architect asserted it" to "Phase 7 verified and recorded it" with a pasted, reproducible command and its real output.

**A6 -- no client recomputes `completionPercent`/`quizPassed`/`courseCompletedAt`/enrollment status from raw data; the one real client-side derivation is `isEnrolled`, and it diverges between clients.** `grep -rn "completionPercent\s*=\|quizPassed\s*=\|courseCompletedAt\s*="` across `web/src`, `mobile/androidApp/src`, `mobile/shared/src` shows every non-test hit is a straight pass-through assignment from the server's `ProgressResponse` (e.g. `CoursePlayerViewModel.kt:574 quizPassed = progress.quizPassed`, `ProgressRepositoryImpl.kt:50-52`) -- never a client-side recomputation from `completedLessonIds.size` or similar. `isEnrolled` is different: `CourseSummary.kt`'s own kdoc states it plainly -- "Deliberately has no `isEnrolled` field -- that has never been implemented anywhere in the backend." Both clients derive it themselves from the enrollment list, and they derive it DIFFERENTLY:
- **Android** (`CourseDetailsViewModel.isEnrolledIn`, kdoc "G4"): pages FULLY through `GET /api/v1/enrollments`, never trusting page 1 alone.
- **Web** (`web/src/lib/api/enrollment.ts` `listEnrollments`): fetches a single page, `?limit=100`, and `useIsEnrolled` checks only that page.

**F1 decision (recommended by the System Design, taken here): record as a disclosed, accepted divergence, not a defect to fix in Phase 7.** A false-negative `isEnrolled` on Web requires a single demo/portfolio account to have enrolled in more than 100 courses -- unreachable in this project's actual demo data (`SeedData.kt` seeds 3 courses total) and not a realistic reviewer/demo scenario. Fixing it (paginating Web's enrollment check the way Android does) is a real, well-understood, small change, but it is not gated by any Phase 7 acceptance criterion and touches production `web/src` code outside T9's own "no code change" scope -- deferred as a disclosed limitation, matching this project's established precedent (Phase 4 D94, Phase 6 F6/F8) for exactly this shape of decision.

**C3 -- Android has zero instructor/admin destinations.** `grep -rniE "instructor|admin" mobile/androidApp/src/main/kotlin/com/mentora/android/navigation/*.kt` and `find mobile/androidApp/src/main -iname "*instructor*" -o -iname "*admin*"` both returned empty. Confirmed clean.

**D2 -- payment-processor scan, word-boundary anchored (Design § 8.2's `alreadyEnrolled` false-positive gotcha in mind).** `grep -rniE "\b(stripe|paypal|braintree|square[_ ]?(up|pay)|adyen|razorpay|paymob|fawry)\b" backend/src web/src mobile/shared/src mobile/androidApp/src` returned exactly 2 hits, both benign: `NoPaymentVocabularyTest.kt` (a test whose OWN purpose is asserting these words appear nowhere else in the shared module -- its source necessarily contains the word list) and `CourseArtwork.kt`'s "vertical **stripe** texture" (the English word, an unrelated UI decoration, not the payment processor). No real payment-processor integration found. Confirmed clean.

**F2 -- no secret anywhere; no Phase 7 diff introduced one.** `backend/.env`'s `AI_PROVIDER_API_KEY=` is blank (comment above it confirms deliberate, D146); `backend/.env.example` matches; `grep -rn "secrets\." .github/workflows/*.yml` returned zero matches across all five workflow files (each file's own header additionally states this explicitly); `AI_PROVIDER_API_KEY` appears in workflow YAML only as the literal empty string `''` (`backend-ci.yml`, `web-ci.yml`); `git log --oneline 6bb0d90..HEAD -- .github/workflows/ backend/ web/ mobile/` (the full Phase 7 commit range) shows only the 13 already-recorded Phase 7 commits, no unexpected additions. Confirmed clean.

**I4 -- `ios-ci.yml` untouched by Phase 7; last iOS CI run still green.** `git log --oneline 6bb0d90..HEAD -- .github/workflows/ios-ci.yml` returned empty (zero Phase 7 commits touched it) -- the file's last real change is `275b8e6`, a Phase 5 CI-infra flake fix, well before the Phase 5 freeze. `gh run list --workflow=ios-ci.yml --limit 3` shows the last 3 runs (all from Phase 5, 2026-09-20T01:2x-01:4x, before the freeze) as `completed / success`. Confirmed clean -- iOS remains frozen exactly as decided, and its last known CI state is green, not broken-and-abandoned.
...
## D159 -- 2026-09-21 -- PHASE 7 T11 done: Android emulator walk, C4 live repro PASS, all A1-A4 spot-checks match T10 exactly; 1 real cross-account data-leak bug found and fixed live; instrumented suite 106 tests, 1 transient failure, 2 consistent failures under separate investigation

**Decision:** delegated the Android portfolio-priority flow walk to a background QA agent driving a real debug APK on the already-running `Chatting_Pixel_8_API_36` (Pixel 8, API 36) emulator via `adb` (mechanism M-AND, the same one Phase 6 T7 used). Backend reached at `10.0.2.2:8080` (the debug build's `ApiEnvironment` was already correctly configured for this — no manual `adb reverse` needed). Logged in as the SAME seeded `student1@mentora.dev` account T10 used on the Website, per the plan.

**Matrix used: en-Light + ar-Dark**, matching T10's own stated reduced matrix (D158, F4). All 9 steps (Discovery -> Course Details -> Demo Checkout -> Purchase Success -> Course Player -> Progress -> Quiz -> AI Tutor -> Certificate) **PASS** on both language/theme runs -- 18/18 step-checks, matching T10's own "18 step-checks" framing. Demo Checkout/Purchase Success/Quiz were exercised against the already-owned/already-passed course (same idempotent-behavior pattern T10 recorded), not re-purchased/re-broken.

**RTL mirroring, concretely observed (ar-Dark):** back arrow flips to point right; bottom nav order fully reversed; Settings dropdown chevrons move left; category filter chips reorder right-to-left; search icon moves to the right end of the bar; course-card carousel arrows flip direction; Purchase Success/course-completed CTA icons mirror left; AI Tutor composer send button and user-message bubbles move left, AI-response bubbles stay right (exact match to T10's own Website RTL description); embedded English content (course titles, instructor names) correctly stays LTR within the RTL page, matching T10's finding. No broken/overlapping layout on either run.

**C4 live repro (mandatory gate for criterion C4) -- PASS, both screens, real before/after:**
| Screen | Guest (before login) | Post-login (after login -> return) |
|---|---|---|
| Course Details | "Login to Enroll" (screenshotted) | "Continue Learning" -- correctly reflects real server enrollment state, no stale gate |
| Learning Path Details | Guest-gated "Follow Path" tap -> Login | Screen re-composes directly showing real progress/"Continue Learning", no stale "Follow Path" |

Both verified live via the `LaunchedEffect(isAuthenticated)`/`onAuthenticationChanged` mechanism T6 added (D154, `0e0b233`) -- this is the mandatory device-level proof that fix actually works, not just its unit tests.

**A1-A4 live spot-checks -- all 4 explicitly match T10's own recorded Website values, not just "looked right":**
| Criterion | T10 (Website) | T11 (Android) | Match |
|---|---|---|---|
| A1 Enrollment | Enrolled, "Building Reliable REST APIs" | Course Details shows "Continue Learning" (enrolled) | Yes |
| A2 Lesson progress | 12/12 lessons, 100% | Course Player: "Lesson 12 of 12 - 100%" | Yes |
| A3 Quiz result | Passed | `quizPassed: true`, course short-circuits to "Course completed!" | Yes |
| A4 Certificate | Issued 2026-09-06, 3 total certs | Certificate detail: same course, same instructor, same date "Sep 6, 2026"; Profile shows 3 certificates | Yes (exact date match) |

**F1** (cheap AI Tutor re-confirm, stub mode): PASS on both runs -- quick-action pills tappable, a real mid-stream partial-text "thinking" state was captured live (not just asserted), pills disable during processing and re-enable after, fully localized in Arabic.

**A genuinely new, real bug found DURING the A1-A4 spot-checks (not a cross-client-parity issue -- a single-client data-integrity defect, found and fixed live because of its severity, matching this project's standing precedent of fixing real defects found along the way -- D148, D151, D154 -- over mechanically forcing every finding into the plan's cross-client (a)/(b)/(c) buckets when a bug is serious enough that leaving it disclosed-but-unfixed would be irresponsible):**

> **Cross-account tab-state leak on My Learning.** Switching from `student2@mentora.dev` (Salma) to `student1@mentora.dev` (Youssef) without a process restart showed **Salma's real per-course progress percentage under Youssef's own identity** on the My Learning tab (Profile's own aggregate counts and `AuthState.Authenticated.user` were always correct throughout -- this was never a stale auth token; `SessionManager`/`AuthPlugin`/`ApiClient` independently confirmed correct). **Root cause:** `MentoraNavHost`'s two full-stack-reset branches (login, logout) only pop the CURRENT back stack (`popUpTo(navController.graph.id){inclusive=true}`) -- they never touch Navigation-Compose's SEPARATE saved-back-stack registry that `onTabTapped`'s switch-tab branch populates via `saveState=true`/`restoreState=true`. Concretely: Account A visits My Learning, switches tabs away (saving that tab's `MyLearningViewModel` instance, already holding Account A's fetched data); Account A logs out; Account B logs in; the FIRST time Account B's session switches to My Learning, `restoreState=true` resurrects that SAME saved instance (its `init` never re-runs) under Account B's identity. **Fixed:** a new `clearAllTabBackStacks()` explicitly clears the saved state for all 5 bottom-nav tab roots (Home/Explore/MyLearning/AiTutor/Profile) at both full-stack-reset sites, plus the pending-intent login branch (which otherwise reset nothing). **Verified live:** a full Salma -> Youssef -> Salma round trip now shows each account's own real progress (8%, 100%, 8% respectively) with no process restart needed. **Commit `ca47fa1`** -- `mobile/androidApp/src/main/kotlin/com/mentora/android/navigation/MentoraNavHost.kt` only, 41 lines added.

**Minor, pre-existing, non-blocking observation, not investigated further (out of scope):** Profile shows "2 courses completed" against "3 certificates" for this account -- a small internal count discrepancy present both before and after this session's fix. Not chased; flagged for awareness only.

**Instrumented test suite (`connectedDebugAndroidTest`) -- closes the T7-review LOW-8 deferral (D155):** 106 tests run, 3 failed.
- `MediaPlaybackControllerInstrumentedTest.realExoPlayer_reachesPlayingState_onARealSeededLessonVideo` -- failed on `RateLimitedAuth (429)` during test setup's own register call, from this session's own extensive manual-testing request volume hitting the real 10-req/min auth bucket. Re-ran in isolation immediately after: **PASSED**. Transient, not a regression -- bucket (b)-equivalent (a known, already-documented real rate limit, `web/e2e/helpers.ts`'s own doc comment describes the identical class of transient failure for the exact same reason).
- `AppDialogColorAndLayoutTest.darkThemeScrimMatchesOverlayScrimTokenExactly` and `.lightThemeScrimMatchesOverlayScrimTokenExactly` -- both failed, and failed IDENTICALLY on a second isolated re-run (not flaky/order-dependent). This exercises the exact mechanism D82 (Phase 4 Task 8) required 3 fix attempts, 3 independent re-verifications, and a Codex second opinion to genuinely fix, and which D82 itself confirmed 65/65 on this SAME emulator (`Chatting_Pixel_8_API_36`) on 2026-09-13. **Not fixed within T11** -- correctly deferred rather than improvising a fix inside an unrelated task, per D82's own demonstrated need for dedicated, multi-attempt rigor here specifically. **Recorded as a new, disclosed, UNRESOLVED finding requiring dedicated follow-up before Phase 7's acceptance audit (T13) can close.** A separate, narrow investigation (isolating whether this is a genuine code regression or an artifact of the emulator's accumulated state from T11's own long manual-testing session on the same live instance) is in progress now; this entry will be followed by its result.

**Files touched.** `mobile/androidApp/src/main/kotlin/com/mentora/android/navigation/MentoraNavHost.kt` (the one real fix, commit `ca47fa1`) only -- consistent with the plan's "Files touched: None" gate except for the one bucket-(a)-equivalent fix, which is its own separate commit as required.

**T11 substantially done; one item open.** Next: resolve the AppDialog scrim-test regression (investigation in progress), then T12 -- demo-environment and documentation accuracy pass.

## D160 -- 2026-09-21 -- AppDialog scrim regression CLOSED for Phase 7 purposes: confirmed real, two bounded fix attempts failed and were cleanly reverted, but the affected component has ZERO current screen consumers -- no demo/user-visible impact today

**Decision:** ran a dedicated, three-stage investigation into the `AppDialogColorAndLayoutTest` failures D159 flagged, following D82's own standard of measuring reality rather than trusting a self-report.

**Stage 1 -- confirm real vs. environmental.** Cold-rebooted the emulator (`adb -s emulator-5554 reboot`, waited for `sys.boot_completed=1`), reinstalled a clean debug APK, and ran ONLY the two failing tests as the very first thing on the freshly-booted device (no prior manual interaction of any kind). **Both failed identically to T11's run** -- `darkThemeScrimMatchesOverlayScrimTokenExactly` measured `Color(0.980,0.980,0.980,1.0)` (expected `~0.361`); `lightThemeScrimMatchesOverlayScrimTokenExactly` measured the same `0.980` (expected `~0.553-0.565`). **Verdict: a real, reproducible regression, not emulator-state contamination.** No code-level cause found (`AppDialog.kt` unchanged since D82's `0f479ef`; `compileSdk`/`targetSdk` = 36 since the project's first Android commit, never changed; no Compose/Activity/Window dependency changed in `libs.versions.toml` since D82). Best-evidenced (unconfirmed) hypothesis: a silent Android emulator system-image update on this bleeding-edge API 36 AVD changed platform-level edge-to-edge/window-dimming behavior between D82's 2026-09-13 verification and now (`ro.build.fingerprint`: `google/sdk_gphone64_x86_64/emu64xa:16/BE2A.250530.026.F3/13894323:userdebug/dev-keys`, `security_patch` 2025-07-05).

**Stage 2 -- two bounded, well-reasoned fix attempts, both failed identically, both cleanly reverted (matching D82's own "guess and hope, then disprove" pattern, but bounded to 2 attempts this time rather than 3, per explicit instruction to stop and disclose rather than chase indefinitely):**
- **Attempt 1** -- disabled `window.isStatusBarContrastEnforced`/`isNavigationBarContrastEnforced` (default `true` on API 29+, which draws a system-owned translucent scrim over the status/nav-bar strips independent of app content -- a plausible second darkening layer distinct from D82's `FLAG_DIM_BEHIND`), guarded by `SDK_INT >= Q`. Result: **bit-for-bit identical failure**, `0.98039216` to 8 decimal places, before and after.
- **Attempt 2** -- hypothesized the dialog window's `LayoutParams` might not actually be `MATCH_PARENT` any more, so `FLAG_LAYOUT_NO_LIMITS` alone no longer forces the window's physical bounds to cover the status-bar strip. Added an explicit `window.setLayout(MATCH_PARENT, MATCH_PARENT)`. Result: **again bit-for-bit identical** -- same `0.98039216`, same failure.

Two independent, differently-targeted window-flag-level mechanisms producing a *bit-for-bit identical* measurement is itself informative: it rules out both hypotheses cleanly (the sampled pixel is not responding to anything this file's `SideEffect` can control at the window-flag level) rather than leaving them ambiguous. No third well-evidenced hypothesis was available within the bounded-effort limit, so the investigation stopped there per instruction. **Both attempts were reverted (`git checkout --`) -- the codebase and the installed APK are unchanged from before this investigation; no commit was made for either failed attempt.**

**Stage 3 -- the decisive finding: `AppDialog` has ZERO real screen consumers anywhere in the shipped app today.** `grep -rn "AppDialog\b" mobile/androidApp/src/main/kotlin` (excluding `AppDialog.kt` itself) shows exactly two hits: a doc-comment cross-reference in `LocalizedContent.kt` and `ComponentKitBPreviews.kt`'s own `@Preview` composable -- **no production screen anywhere calls `AppDialog(...)`.** It was built ahead of its consumers in Phase 4 Task 8 (a foundational component-kit task, D82) and nothing has adopted it since. By contrast, `MentoraBottomSheet` -- the other component D82's investigation and fix touched -- IS in real production use (`CurriculumBottomSheet.kt`, the Course Player's curriculum sheet, walked live and screenshotted by both T10 and T11 with no dimming defect observed) but is built on Material3's own `ModalBottomSheet` and its standard, independent `scrimColor` mechanism -- confirmed by reading `MentoraBottomSheet.kt` directly -- **not** `AppDialog`'s custom `setDimAmount`/`DialogWindowProvider`/`decorFitsSystemWindows` workaround. It is architecturally unaffected by this regression, consistent with T10/T11 never reporting a dimming defect on the one real dialog-like surface either walk actually exercised.

**Conclusion: this regression is real and confirmed, but has ZERO current demo-visible or user-visible impact**, because nothing in the shipped app currently renders an `AppDialog`. It does not block Phase 7's demo-readiness or acceptance for any currently-existing screen. **Recorded as an accepted, unresolved, dormant defect requiring dedicated future investigation before any future task wires `AppDialog` into a real screen** -- matching D82's own "4th attempt not pursued" disclosure precedent, not silently forgotten. The next investigator's best untried leads (from the fix-attempt agent's own final report, not yet tested): check whether this is specific to the *instrumented-test harness's own synthetic host Activity* (which may not itself be edge-to-edge/theme-configured identically to a real launched app Activity) by adding a manual/visual check in the actually-running app; or use `adb shell dumpsys window windows` (D82's own original debugging tool) to inspect the live window hierarchy and identify what is actually compositing at the measured pixel.
...
## D162 -- 2026-09-21 -- PHASE 7 T13 CLOSES: full A-J acceptance audit complete, zero regressions, PHASE 7 COMPLETE

**Decision:** ran the final criterion-by-criterion acceptance audit over every row of `execution/PHASE_7_ACCEPTANCE_CRITERIA.md` sections A-J, updated in place with evidence citations, not a blanket "looks done" declaration -- the same rigor Phase 6's T9 audit applied.

**J1 -- full suite re-run FRESH (`--rerun-tasks`, not cached) vs. the T0 baseline (backend 127/127, `:shared` 249/249, `:androidApp` JVM 241/241, lint 0 errors, Playwright chromium 19/19):** backend **127/127** (matches), `:shared` **249/249** (matches), `:androidApp` JVM **246/246** (EXCEEDS baseline -- the 5 new tests T6/T11's C4-fix work added), `:androidApp:lintDebug` **0 errors** (matches), Playwright chromium **19/19** under `CI=true` (matches, one self-healing flake exactly as T0 itself documented). **Zero regressions anywhere.** One methodology note for the record: an initial local run with no `CI` env var showed 7 failures, but these were fully explained and non-regressive (5 genuine auth-rate-limit exhaustion from this session's own unusually heavy cumulative testing across T8/T10/T11, confirmed transient by re-running minutes later; 2 the pre-existing timing-flake class T0 already documented) -- re-running under `CI=true`, the representative environment real CI itself uses, gave the accurate, clean picture.

**J2 -- all 5 CI workflows confirmed green on the repository's real final state:** each workflow's most recent real run remains green and covers the current state of every path it watches -- no relevant file has changed since `backend-ci.yml`'s last run (T3), `web-ci.yml`'s last run (T7), `tokens-ci.yml`'s last run (T7), or `ios-ci.yml`'s last run (Phase 5, pre-freeze); `android-ci.yml`'s last run (T11) already covers the most recent `mobile/androidApp/**` change.

**J3 -- documentation updated to Phase 7's real end state:** `execution/CURRENT_STATUS.md`'s top banner and Phase 7 task table (all T0-T13 rows now DONE), plus its stale bottom-of-file phase-status table corrected (Phase 2/5/6/7 rows had drifted out of sync with reality over multiple phases -- Phase 2 said "IN_PROGRESS" though completed phases ago, Phase 5 said "IN_PROGRESS" instead of reflecting the freeze, Phase 6/7 said "NOT_STARTED" though both are now complete -- fixed as a real accuracy correction, not new scope); `execution/PHASE_HANDOFF.md`'s Phase 7 entry added in the established 10-subsection format; `execution/PHASE_7_ACCEPTANCE_CRITERIA.md`'s full A-J matrix updated in place plus a new "Closing state confirmed at T13" section.

**J4 -- clean git state, one commit per task, throughout:** confirmed via `git log --oneline 60e71b7..HEAD` -- 27 commits spanning Phase 7 kickoff (`9b2a1c9`) through T12 (`eb137d7`), every task's own commit(s) pushed immediately after, `git status --porcelain` clean at every checkpoint this audit re-traced.

**J5 -- every fix traceable, restated:** T6's C4 fix (disclosed cross-client auth-consistency bug), T0's 2 E2E fixes (I2/B1), T1's gradlew fix and T3's shadowJar fix (both block a locked M16 CI deliverable), T11's cross-account tab-leak fix (a data-integrity defect serious enough to fix despite not being strictly "cross-client", explicitly reasoned in D159). Two candidate fixes were correctly NOT made because they didn't trace to anything in scope: the quiz-badge CSS bug (D158) and the `AppDialog` regression (D160) -- both disclosed, neither fixed, matching J5's own scope-discipline rule.

**A6, C3, D2 marked verified-and-recorded, citing T9's evidence (D157) -- not re-verified from scratch, per the plan's own instruction.**

**Full A-J matrix result: every row is PASS, PASS-with-a-disclosed-exception (A6's F1 divergence, E2's quiz-badge CSS bug), or an already-accepted/held constraint (F2, I6, G3, G4). Nothing was upgraded to PASS to close the phase** -- the two genuinely open items (F1, the `AppDialog` regression) remain honestly recorded as disclosed, non-blocking limitations in `PHASE_HANDOFF.md`'s § 6, not silently dropped and not fabricated as resolved.

**Files touched.** `execution/PHASE_7_ACCEPTANCE_CRITERIA.md`, `execution/CURRENT_STATUS.md`, `execution/DECISIONS_LOG.md` (this entry), `execution/PHASE_HANDOFF.md` -- exactly the plan's own stated file list for T13.

**PHASE 7 IS NOW COMPLETE.** Summary: 5 real, green CI workflows stood up from nothing; a disclosed cross-client bug (C4) fixed and live-verified on a real device; a new cross-client parity harness proving Website/Android/KMP observe identical backend state; full live portfolio-priority walks on both Website and Android, in English and Arabic, Light and Dark; one genuinely serious pre-existing infra bug found and fixed (the backend fat jar was never runnable, D151); one real data-integrity bug found and fixed live (Android's cross-account tab-state leak, D159); one real regression investigated to ground truth and correctly closed as a disclosed, zero-current-impact dormant defect rather than either silently dropped or improvised-fixed (D160); documentation brought back into exact agreement with reality, including correcting several phases' worth of accumulated drift in `CURRENT_STATUS.md`'s own summary table.

**PHASE 8 MUST NOT BEGIN WITHOUT THE USER'S OWN SEPARATE, EXPLICIT APPROVAL** -- this is a hard stop restated one more time, per the standing phase-execution policy, not a formality to skip past.
