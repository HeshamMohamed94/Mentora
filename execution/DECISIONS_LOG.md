...
## D121 — 2026-09-19 — T6 slice 3b (theme-root wiring) implemented, CI round 1 RED (test-only), fixed
...
**Fix applied**: `numberStyle` reverted to `.none` (the original, never the real issue). The negative control now uses an EXPLICIT `Locale(identifier: "ar-u-nu-arab")` rather than relying on bare `"ar"`'s default numbering system — deterministic across ICU/CLDR versions and vendors, and it upgrades the control into the check that actually matters: proving `NumberFormatter` honors the `-u-nu-*` Unicode extension mechanism at all (the exact mechanism `arabicLocaleIdentifier`'s `-u-nu-latn` depends on), rather than asserting anything about a locale default. Also added, per the same review: a direct, deterministic typo guard in `test_arabicLocaleKeepsArabicLanguageSubtag` asserting `arabicLocaleIdentifier.contains("-u-nu-latn")` — the digit-rendering test alone can no longer serve as an indirect guard on Apple platforms, now that bare `"ar"` is confirmed to already resolve to Latin digits there. Corrected the false "bare `ar` renders Eastern digits" claim everywhere it appeared (this file, above, and `MentoraTheme.swift`'s `arabicLocaleIdentifier` doc comment) — the override's real justification is region-specific Arabic locales (`ar_EG`/`ar_SA`, which DO default to `arab` even on Apple platforms) and `architecture/LOCALIZATION_ARCHITECTURE.md`'s unconditional cross-platform requirement (Web/Android genuinely need it), not a universal claim about every `"ar"`-tagged locale on every platform.

**Status: PENDING CI (second push).** Not yet pushed/compiled with this fix. The next real `ios-ci.yml` run is this slice's actual verification (compile of `MentoraTheme.swift`, `LocaleController.swift`'s non-optional seed, `MentoraApp.swift`'s `MentoraRootView`, and all 6 `MentoraThemeTests` cases, now including the corrected numeral-system test and the new typo guard). T6 slice 3c (token gallery + completion-gate script) is next, once this slice is real-CI-green.
