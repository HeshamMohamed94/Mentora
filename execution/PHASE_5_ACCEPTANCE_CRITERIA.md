...
`W→M` even when its subject matter (a string table, a mapping table) looks static. Where a criterion
...
Four named Mac checkpoints are defined in `PHASE_5_IOS_IMPLEMENTATION_PLAN.md § 3`
...
| F5 | On a Mac: `:shared:compileKotlinIosSimulatorArm64`, `:shared:linkDebugFrameworkIosSimulatorArm64`, SKIE actually applying, and the XCFramework assembling all succeed — the first genuine verification that D69's disclosed limitation is closed. | D69 / limitation B1 | Full command output captured into the handoff | M (MC-1) |
...
| J10 | The phase-completion report states unambiguously which criteria ended as **NOT TESTABLE (HOST)** if no Mac ever became available, rather than implying coverage that does not exist. | Methodology § 2 | Final acceptance report | W |
...
1. **Is a macOS host with Xcode actually available?** If not, Phase 5 can be *authored* but not
   built, run, tested, or accepted — every M-tagged criterion above (the large majority) would end as
