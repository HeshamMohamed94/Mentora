...
**Next immediate action:** Task 10 — Course Details. See `execution/PHASE_4_ANDROID_PLAN.md` § 5,
Task T10 for exact acceptance criteria. Note `Destination.LearningPaths` no longer exists (folded
into Explore's own tab state in Task 9) — Course Details' enrollment-aware CTA still routes through
`Destination.CourseDetails`/`DemoCheckout`/`CoursePlayer`, unaffected. Known disclosed limitation to
...
`CourseCard` has no price slot (Task 8 gap, inherited by Task 9) — Course Details will need its own
price rendering since it can't reuse that component's absence.
