...
| 16 | Learning Path Details (follow/unfollow) | NOT STARTED |
| 17 | AI Tutor (streaming chat, stub provider) | NOT STARTED |
...
**Next immediate action:** Task 16 — Learning Path Details (follow/unfollow). `Destination
.LearningPathDetails` currently resolves to `PlaceholderScreens.kt`'s placeholder; Explore's own
Learning Paths tab (Task 9) already lists real paths and navigates to this destination on tap. Check
`sdk.learningPaths.*`'s (or equivalent facade name — confirm the real one, don't assume) actual method
surface before implementation, same "research the shared API first" discipline Tasks 14/15 both used.
**Navigation-registration checklist for this and every remaining task**: Task 15's own D90 found a
real HIGH bug — a destination reachable from a shared, multi-graph-registered content lambda was
missing its OWN registration under one of those graphs (`CertificateDetail` under `ExploreGraph`),
silently mis-anchoring the bottom nav and risking a whole-tab-stack loss on the next same-tab tap.
Before wiring any new destination reachable from more than one already-multi-graph screen, grep every
graph block in `MentoraNavHost.kt` that registers the PARENT screen and confirm the new CHILD
destination is registered in every one of them, not just the one the implementer happened to test from.
**Standing
