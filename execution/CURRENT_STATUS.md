...
**Exact next task to resume on: Task 3** (environment config + Ktor `HttpClient` factory +
`ApiClient`). Full acceptance criteria for every task are in `execution/PHASE_3_KMP_PLAN.md` (see
D69) — read that before resuming a task, do not re-derive from memory.
