...
**Exact next task to resume on: Task 15** (Public façade, Koin wiring, iOS framework export + SKIE,
host-guarded). Full acceptance criteria are in `execution/PHASE_3_KMP_PLAN.md` (see D69) — read that
before resuming. This is the task that wires everything built in Tasks 1-14 (every repository/use
case/manager) into a single Koin DI graph and a clean public façade for Phase 4/5 to consume, plus
the iOS framework export config (SKIE must be applied host-guarded — macOS only, per disclosed
limitation B1; this Windows machine cannot compile or verify the iOS side).
