...
Task 11: Instructor Web — Dashboard, Course Editor (Overview/Curriculum), Lesson Editor, Quiz
Editor. Read the real backend source before assuming any endpoint shape (same discipline as every
prior task): `backend/src/main/kotlin/com/mentora/backend/courses/` for course/section/lesson
CRUD, the instructor aggregation endpoint mentioned in the Phase 1 summary
(`execution/PHASE_HANDOFF.md § 8`-area notes on an "Instructor aggregation endpoint, M11 tail"),
and the `quiz`/`media` modules for how an instructor authors/uploads quiz questions and lesson
video. Check `product/SCREEN_INVENTORY.md`/`design-system/COMPONENTS.md` for Instructor-specific
components not yet built (e.g. `DataTable`, referenced in D40/D41 as an existing popover
convention but not yet confirmed as a built React component) before assuming the current
`web/src/components/ui/` kit already covers this surface. The new `Select` component from task 10
(D44) is very likely reusable here for Course Editor's Category/Level/Content-Language fields —
confirm before building a second one.

**Per explicit user instruction (2026-09-06 session): Task 10 is the last task completed this
session. Do not start Task 11 without a new go-ahead — this note is for whenever that go-ahead
comes.**
