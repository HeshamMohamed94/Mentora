...
| T4b | Swift app bootstrap (SDK/session/locale/theme wiring) | **IMPLEMENTED, FIX ROUND APPLIED — PENDING CI (not DONE).** Authored on Windows against the real CI-run-#6 `kmp-swift-interface` artifact (`shared.h` + SKIE-generated `Shared/*.swift`), not guessed. `MentoraApp.swift` rewritten; new `Support/AppEnvironment.swift`, `SessionController.swift`, `LocaleController.swift`, `ThemeController.swift`, `KotlinFlowBridge.swift`. See `DECISIONS_LOG.md` D108 for full detail, including a disclosed Flow-bridging deviation from System Design § 5/§ 7 (the real interface exposes plain `Kotlinx_coroutines_core{State,Shared}Flow`, not a SKIE `AsyncSequence`, for `observeAuthState`/`observeLocale`/`KeychainStatus.failures` — fixed via a small generic `KotlinFlowBridge` helper) and a second, self-discovered deviation (`SetLocaleUseCase.invoke(locale:)` is itself a suspend function). **A pre-CI Opus review of this diff against the real Kotlin sources then found 9 genuine bugs (1 critical, 2 likely compile blockers) — all fixed in a same-day D108 "Fix round" before any CI spend**: `KotlinFlowWatcher.emit`'s unenforced cross-thread `@MainActor` write (now `nonisolated` + explicit `DispatchQueue.main.async`), `import Shared` → `import shared` (module-name case mismatch, all 5 files), unqualified top-level Kotlin function calls (`PlatformModule_iosKt.platformModule()`, `LocaleResolverKt.resolveInitialLocale(...)`), `watchKotlinFlow`'s non-owning `Task` (switched to the `async throws collect` form), a redundant racing `sessionController.apply()` call removed, stale `profile` on logout now cleared, and a durable `UserDefaults.synchronize()` flush added for the locale-seed flag (Android `commit()` precedent). `:shared:testDebugUnitTest`/`:androidApp:testDebugUnitTest` re-confirmed unaffected both before and after the fix round (no Kotlin touched either time). No Swift can compile/link/run on this Windows host — a real green `ios-ci.yml` run (MC-1) is required before this moves to DONE, and specifically needs to confirm the `async throws collect` call compiles and whether a zero-arg `iosSimulator()` SKIE overload exists (both flagged as open in D108); live behavior (Keychain round-trip, session restore, actual simulator rendering) still needs MC-2/MC-3 after that. |
