...
**Last updated:** 2026-09-11 (Task 13 — Localization completion pass + RTL QA sweep — DONE, D63; Phase 2 continuing automatically to Task 14 per the active Phase Execution Policy)
...
Task 14: Playwright E2E suite (M15 web portion) — `@playwright/test` is already a declared
dependency and a `test:e2e` script exists, but no `playwright.config.ts` and no test files exist
yet (genuinely NOT_STARTED). Build against `architecture/TESTING_STRATEGY.md § 6`'s 10 CI-blocking
priority flows, including "Admin Course Management," which now has a real Admin Web (Task 12) to
exercise. Then Task 15 (`web/README.md`), Task 16 (Phase 2 quality gate), Task 17
