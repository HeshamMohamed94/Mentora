...
**Next immediate action:** Task 13 — `LessonPlaybackController` (ExoPlayer/Media3) + Course Player +
Curriculum Bottom Sheet. See `execution/PHASE_4_ANDROID_PLAN.md` § 5, Task T13 for exact acceptance
criteria — flagged by the plan itself as the highest-risk remaining task (exact-showcase + ux-only,
real media playback). **Standing requirement for every remaining screen task**: add real
