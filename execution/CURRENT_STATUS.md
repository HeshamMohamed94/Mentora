...
| T4b | Swift app bootstrap (SDK/session/locale/theme wiring) | **NOT STARTED — now genuinely unblocked.** CI-1 is green; next step is downloading/reading the `kmp-swift-interface` artifact from run #6 before writing any bridge code, then authoring this task's slice. Live behavior (Keychain round-trip, session restore) still needs MC-2 on an actual simulator/device. |
