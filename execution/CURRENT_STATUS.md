...
| 12 | Home + My Learning (+ Certificates entry) | NOT STARTED |
| 13 | `LessonPlaybackController` (ExoPlayer/Media3) + Course Player + Curriculum Bottom Sheet | NOT STARTED |
| 14 | Quiz + Quiz Results | NOT STARTED |
...
**Next immediate action:** Task 12 — Home + My Learning (+ Certificates entry). See
`execution/PHASE_4_ANDROID_PLAN.md` § 5, Task T12 for exact acceptance criteria. **Standing
requirement for every remaining screen task**: add real `values-ar/strings.xml` translations for
every new string resource AS PART OF that task's own deliverable, not deferred to Task 19 — Tasks
9-10 briefly accumulated a 29-string EN/AR gap that had to be backfilled as a same-session
follow-up; do not let this recur. Known disclosed limitation to carry forward: `AppDialog`'s scrim
may flash briefly over-dimmed on open before settling to the correct steady-state color (see Task 8
entry) — not blocking, not yet re-verified further. `CourseCard` has no price slot (Task 8 gap) —
Course Details built its own inline price formatting rather than extending that component.
**Test-infrastructure note for any future instrumented test work**: `NavigationShellTest`'s
`registerFreshRealStudent()` accumulates throwaway `t11-navshelltest-*@example.com` accounts in the
shared local dev MongoDB on every real run (no delete-account/unenroll endpoint exists to call from
an instrumented test) — sweep manually via `mongosh` before using that database for a demo, same
precedent as Task 7's own cleanup.
