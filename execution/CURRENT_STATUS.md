...
| PHASE 4 — Android | **IN_PROGRESS** | Started 2026-09-13, approved by the user 2026-09-13. See `execution/PHASE_4_ANDROID_PLAN.md` and the task breakdown near the end of this file. |
...
| 18 | Profile + Settings (language selector, theme, logout) | NOT STARTED |
...
**Next immediate action:** Task 18 — Profile + Settings (language selector, theme, logout).
`Destination.Profile`/`Destination.Settings` currently resolve to `PlaceholderScreens.kt` placeholders
(`ProfileScreen` already has a real `onOpenSettings` wired to `PrimaryButton`, but no real content).
Research `sdk.user.*`/`sdk.auth.*`'s real method surface (profile fetch/update, logout) and
`AndroidPreferenceStore`/`ThemeController`/`LocaleController`'s existing bootstrap wiring (Task 4) before
implementation — Settings' theme/language controls must integrate with that already-established
application-scoped state, not invent a parallel mechanism. Check `design-to-code/screens/` for any
exact-showcase Profile/Settings mobile spec before falling back to `ux/SCREEN_UX_SPECS.md §§ 16-17` (no
exact-showcase mobile Profile/Settings frame was extracted in Task 3 per that task's own account — verify
this is still true before assuming `ux-only`). Web's Task 10 precedent (D44) disclosed two real
spec-vs-backend gaps worth re-checking here: no password-change endpoint exists, and `avatarMediaId` is a
dead field nothing ever writes (Profile likely stays initials-only, no upload control, same as Web).
**Navigation-registration checklist, now proven three times (D90, D91, D92)**: before wiring any new
...
has now caught one real HIGH bug (D90) and been proactively applied and verified clean twice since (D91,
D92) — keep doing it. **Instrumented-test-run note (new, from Task 17):** a `connectedDebugAndroidTest`
run can fail on a Gradle/Windows tooling error unrelated to any test itself (an MD5-hash failure on a
stale output file, or a locked logcat-output file left by an orphaned daemon after a prior aborted run) —
if the failure is in Gradle's own task execution (not a JUnit test result) immediately after a prior
interrupted/aborted instrumented run, try `./gradlew.bat --stop` + clearing
`androidApp/build/outputs/androidTest-results`/`androidApp/build/reports/androidTests` before assuming a
real regression and re-diagnosing further. **Standing
requirement for every remaining screen task**: add real
`values-ar/strings.xml` translations for every new string resource AS PART OF that task's own
deliverable, not deferred to Task 19 — Tasks 9-10 briefly accumulated a 29-string EN/AR gap that had
to be backfilled as a same-session follow-up; do not let this recur. Known disclosed limitation to
...
any real code change — confirmed again in Task 12 (D84); kill and relaunch the AVD fresh if a
full-suite run shows failures only in tests unrelated to what you just changed, before assuming a
regression.
