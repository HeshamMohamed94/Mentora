...
- **Current task:** Tasks 1-5 (foundation, auth screens, public discovery, demo checkout/purchase success, dashboard/my learning + the real authenticated Sidebar shell) are DONE and verified end-to-end (real browser + real backend, en+ar). Task 6 (Course Player + Quiz + Quiz Results) is next. Icon set is a hand-drawn inline-SVG placeholder for the real self-hosted Material Symbols Rounded font (D40) — swap later behind the same `Icon` component API, no call-site changes needed. Landing page's own `CourseCard`/`LearningPathCard` usage still hardcodes the Guest `basePath` (unchanged from D39) — low priority since Landing is conceptually Guest-only.
...
| 7 | Certificates List + Certificate Detail | NOT_STARTED — next up; "View Certificate" links from Course Player/Quiz Results already point at `/app/certificates` (forward reference, same pattern as every prior task) |
...
Task 7: Certificates List + Certificate Detail, against the live `certificates` endpoints (`GET /api/v1/certificates` list + `GET /api/v1/certificates/:id` detail — read `backend/src/main/kotlin/com/mentora/backend/certificates/routes/CertificateRoutes.kt` and `CertificateService.kt` for the exact as-built shapes before assuming anything, same discipline as task 6). Routes: `/app/certificates` (grid of `CertificateCard`s, `EmptyState` if none) and `/app/certificates/:id` (detail: student name, course title, instructor, completion date, UI-only "Share" affordance). The certificate's public id format is `MTR-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX` (see `CertificateService.publicId`) — that's what `:id` in the route actually is, not a raw Mongo ObjectId. Once built, go back and give the "View Certificate" links already added in task 6 (Course Player/Quiz Results completion confirmations, currently pointing at the bare `/app/certificates` list) a real target if a specific certificate id is cheaply available at that point — otherwise the list is a fine landing spot and no change is needed there.
