...
| 15 | Certificates List + Certificate Detail | NOT STARTED |
...
**Next immediate action:** Task 15 — Certificates List + Certificate Detail. Both routes
(`Destination.Certificates`/`Destination.CertificateDetail`, already registered in `MentoraNavHost.kt`
under multiple graphs per the established shared-lambda-across-graphs pattern) currently resolve to
`PlaceholderScreens.kt` placeholders. `CertificateCard` already exists from Task 8's component kit
(disclosed placeholder "document" preview, no real certificate asset anywhere in the repo — same D42
precedent as web) — reuse it, don't rebuild it. Multiple already-shipped screens link to
`Destination.Certificates` with no real target yet: `CoursePlayerScreen`'s `onOpenCertificates`
(Course Completed state), `QuizResultsScreen`'s `onOpenCertificates` (the completion banner), and
`HomeScreen`'s header icon (Task 12) — check `sdk.certificates.*`'s actual method surface (mirroring
how Task 14 first researched `sdk.quiz.*` before implementation) rather than assuming its shape.
**Standing
