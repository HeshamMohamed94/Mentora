...
| T7 | Localization foundation: `Localizable.xcstrings` (en+ar, 279 keys), locale/direction environment, `MentoraStrings`, formatters, Node parity+specifier lint | **IN PROGRESS — slices 1-3 of 4 DONE, CI-GREEN.** See "T7 SLICE 1/2/3" sections below. Slice 4 (`localization-checks.js`, T7's last slice) not started. |
| T8-T23 | Component kits through final acceptance audit | **NOT STARTED** — authored on Windows, compiled by CI, one small slice per push; live/visual verification still needs a Mac (MC-3/MC-4). |
