...
**Next immediate action:** Task 8 — Core component kit B (cards, state patterns, sheets). See
`execution/PHASE_4_ANDROID_PLAN.md` § 5, Task T8 for exact acceptance criteria. Note `SearchField`
already exists (built in Task 5) — reuse it, do not rebuild it in Task 9. `ui/error/ApiErrorCopy.kt`
(Task 7) is the central `ApiErrorCode` → localized-copy map every later task handling API errors
should import, not rebuild.
