...
**Next immediate action:** Task 14 — Quiz + Quiz Results. `Destination.Quiz` currently resolves to
`PlaceholderScreens.kt`'s placeholder; Task 13's own `CoursePlayerCompletedContent` (D88) already
depends on it existing for real — a `quizNotYetPassed` branch renders a stale `quizPassed` snapshot on
a quiz round-trip specifically because there is no real Quiz screen yet to make that unreachable-today
gap reachable. Task 14 MUST add a refresh-on-return (re-fetch/reload course progress when popping back
from a passed quiz) or that C3 branch will render wrong the moment it becomes reachable — see the kdoc
comment at `CoursePlayerScreen.kt`'s `quizNotYetPassed` declaration for the exact call site. **Standing
