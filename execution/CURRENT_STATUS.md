...
**Exact next task to resume on: Task 14** (AI Tutor — paged conversation + streaming send + quick
actions). Full acceptance criteria are in `execution/PHASE_3_KMP_PLAN.md` (see D69) — read that
before resuming, especially Decisions D-D (the `AiQuickAction` identity/pairing-rule boundary —
`shared` never owns localized prompt text) and D-E (the raw `text/plain` chunked streaming response
exposed as `Flow<String>`, which bypasses the generic `ApiClient` entirely). No real AI
provider/key/model name may appear anywhere in `shared` — all traffic goes through the Phase 1 stub
`/ai-tutor/*` endpoints.
