...
| 7 | Auth screens (Login, Register) | NOT STARTED |
| 8 | Core component kit B (cards, state patterns, sheets) | NOT STARTED |
| 9 | Explore (+ Learning Paths segment) | NOT STARTED |
| 10 | Course Details | NOT STARTED |
...
**Next immediate action:** Task 7 — Auth screens (Login, Register). See
`execution/PHASE_4_ANDROID_PLAN.md` § 5, Task T7 for exact acceptance criteria. Note `SearchField`
already exists (built in Task 5) — reuse it, do not rebuild it in Task 9. The navigation graph,
routes, and auth-gate mechanism already exist (Task 6) — Task 7 replaces the `Login`/`Register`
placeholder composables with real screens wired to the already-correct `requireAuth`/
`pendingNavIntent` mechanism; it should not need to touch `MentoraNavHost.kt`'s graph structure.
