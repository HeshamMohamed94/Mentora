...
**Last updated:** 2026-09-11 (Task 14 — Playwright E2E suite — DONE, D64; Phase 2 continuing automatically to Task 15 per the active Phase Execution Policy)
...
Task 15: `web/README.md` local run instructions (M16 web portion) — does not exist yet. Should
cover: `start-mentora.ps1`/`stop-mentora.ps1` usage, seed credentials, and how to run the Playwright
suite (`npx playwright test` — currently `chromium`/`firefox` are the two verified-green projects;
`webkit` is configured but blocked by the disclosed cookie-policy limitation in `DECISIONS_LOG.md`
D64/`web/e2e/helpers.ts`, so don't claim it passes). Then Task 16 (Phase 2 quality gate), Task 17
(`PHASE_HANDOFF.md` Phase 2 write-up) — per the active Phase Execution Policy (continue
