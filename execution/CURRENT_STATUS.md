...
**Exact next task to resume on: Task 17** (Documentation & Phase 3 → Phase 4 handoff — the final
task). Full acceptance criteria are in `execution/PHASE_3_KMP_PLAN.md` (see D69, around line 213) —
read that before resuming. Must include: `mobile/README.md` + `mobile/shared/README.md`;
`execution/INTEGRATION_CONTRACT.md` Phase 3 as-built section (additive only — including the
`AUTH_TOKEN_INVALID`/`AUTH_TOKEN_EXPIRED` drift from D77, and every other drift/decision recorded in
D69-D77); `execution/PHASE_HANDOFF.md` Phase 3 entry; `execution/DECISIONS_LOG.md` continuation;
`execution/CURRENT_STATUS.md` final Phase 3 → COMPLETE update; known-limitations disclosure (iOS/SKIE
never compiled here, `androidMain` actuals tested as JVM unit tests only, no offline cache, the
`?language=` dual-role semantics, unpaginated categories/learning-paths, etc.); a
`git log --name-only` gate confirming no Phase 3 commit touched `architecture/`, `product/`, `ux/`,
`design-system/`, `design-review-locked/`, `backend/`, or `web/`. This is a documentation-only task —
no new `shared/` source code. After this, the full Phase 3 completion gate (24-item checklist) runs,
followed by the final "MENTORA PHASE 3 COMPLETE" report and a stop for the user's explicit approval
before Phase 4.
