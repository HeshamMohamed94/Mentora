...
**Exact next task to resume on: Task 16** (Live integration verification against the running local
backend). Full acceptance criteria are in `execution/PHASE_3_KMP_PLAN.md` (see D69) — read that
before resuming. This task requires the local backend + seeded MongoDB to actually be running;
verify reachability (`GET /healthz`) before dispatching/running, and skip cleanly with a clear
message (not a hard failure) if it isn't reachable. Full student journey: register→login→
categories→search→course detail→checkout preview→complete checkout→progress→complete a lesson→
playback-url→quiz→submit→certificate→learning-path follow→AI Tutor stream→PATCH locale→logout. Token
refresh must be exercised for real (force-expire, assert 1 refresh + 1 retry + success). Test
accounts use a distinct greppable pattern (e.g. `@kmp.mentora.test`) and must be cleaned up after —
no mutating production/shared data left behind.
