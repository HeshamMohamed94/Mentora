...
| 6 | Course Player + Quiz + Quiz Results | NOT_STARTED — next up |
...
Task 6: Course Player + Quiz + Quiz Results. Course Player needs a real `/app/learn/:id` route (currently a dead link from Course Details' "Continue Learning"/"Resume" CTAs and My Learning/Dashboard's "Resume"/"Continue" buttons — same pre-existing-forward-reference pattern as checkout was before task 4). Per ux/WEB_UX.md § "Course Player", the Sidebar collapses by default on this screen to maximize width for the two-column video+curriculum layout — `AppShell`'s `collapsed` state is currently a manual user toggle only; Course Player will need a way to force/suggest collapse on mount (e.g. an per-page collapse hint, or the screen just accepts the sidebar's current state — decide when building it). Quiz needs backend quiz-taking endpoints (submit attempt, get results) — check `backend/src/main/kotlin/com/mentora/backend/quiz/` for the as-built shape before assuming anything.
