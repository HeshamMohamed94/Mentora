...
**Next immediate action:** Task 17 — AI Tutor (streaming chat, stub provider). `Destination.AiTutor`
currently resolves to `PlaceholderScreens.kt`'s placeholder. Check `sdk.aiTutor.*`'s (or equivalent
facade name — confirm the real one) actual method surface AND whether the backend's own provider is
genuinely a stub (no real LLM call) before implementation, same "research the shared API first"
discipline every task since Task 14 has used — streaming responses in particular need investigating
for how `shared` exposes them to Compose (a `Flow`? repeated polling? check before assuming either).
**Navigation-registration checklist, now proven twice (D90, D91)**: before wiring any new destination
reachable from more than one already-multi-graph screen, compute the FULL transitive push closure from
that destination (not just its immediate child) and grep every graph block in `MentoraNavHost.kt` that
registers each screen in that closure, confirming every one is registered under every graph the CHAIN
is reachable from — not just the one the implementer happened to test from. This has now caught one
real HIGH bug (D90) and been proactively applied and verified clean once (D91) — keep doing it.
**Standing
