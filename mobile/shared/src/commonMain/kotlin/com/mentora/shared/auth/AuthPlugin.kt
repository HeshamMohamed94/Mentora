 ...
 *    `error.code == "AUTH_TOKEN_EXPIRED"`, calls [SessionManager.refreshAccessToken]
 *    (single-flight — see its kdoc) and, on success, retries the SAME request once with the new
 *    token. A refresh failure, or any other 401 reason (e.g. `AUTH_TOKEN_INVALID` or a login's
 *    `AUTH_INVALID_CREDENTIALS`), returns the original response untouched — never a silent retry
 *    loop, never a second refresh attempt for the same failure.
 */
fun HttpClient.installAuthInterception(sessionManager: SessionManager): HttpClient {
    plugin(HttpSend).intercept { request ->
        ...
        if (call.response.status == HttpStatusCode.Unauthorized) {
            ...
            if (errorCode == AUTH_TOKEN_EXPIRED_CODE) {
